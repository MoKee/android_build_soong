...
var (
	...
	mokeeResTag    = dependencyTag{name: "org.mokee.platform-res"}
...
func (j *Module) deps(ctx android.BottomUpMutatorContext) {
	if ctx.Device() {
		...
		if ctx.ModuleName() == "org.mokee.platform-res" {
			ctx.AddDependency(ctx.Module(), frameworkResTag, "framework-res")
		}
		if ctx.ModuleName() == "org.mokee.platform" || ctx.ModuleName() == "org.mokee.platform.internal" {
			ctx.AddDependency(ctx.Module(), mokeeResTag, "org.mokee.platform-res")
		}
	}
...
func (j *Module) collectDeps(ctx android.ModuleContext) deps {
	...
	ctx.VisitDirectDeps(func(module android.Module) {
		...
		case Dependency:
			...
			case mokeeResTag:
				if ctx.ModuleName() == "org.mokee.platform" ||
					ctx.ModuleName() == "org.mokee.platform.internal" {
					// org.mokee.platform.jar has a one-off dependency on the R.java and Manifest.java files
					// generated by org.mokee.platform-res.apk
					deps.srcJars = append(deps.srcJars, dep.(*AndroidApp).aaptSrcJar)
				}
